...
*/
			...
			*/
			...
			$fuser->contact_id=0;
			$fuser->fk_member=0;
			//$fuser->fk_user=$ldapuser['manager'];
			$fuser->photo = '';
			//$fuser->office_phone = $ldapuser['homePhone'];
			...
			/* Problem on directory creation*/
			if(strtoupper($key) == 'ALCE') {
				//if(! empty($ldapuser['thumbnailPhoto'])) {
				$photo = $ldapuser['thumbnailPhoto'];
				$dir = $conf->user->dir_output.'/'.get_exdir($fuser->id,2,0,1);
                
            	$im = imagecreatefromstring($photo);
				//var_dump($im);exit;
            	$res = dol_mkdir($dir);
            	var_dump($dir, $res);exit;
	            //}

				if(! is_dir($conf->user->dir_output.'/'.$fuser->id)) {
					//dol_mkdir($conf->user->dir_output.'/'.$fuser->id);
					imagejpeg($im, $conf->user->dir_output.'/'.$fuser->id.'/thumbnailPhoto.jpg');
				}
			}
			/**/
		}
