...
print '<td class="right" colspan=2 >'.(empty($conf->global->PROJECT_SHOW_FORECAST_PROFIT_BOARD) ? $langs->trans("Profit") : $langs->trans("RealProfit")).'</td>';
...
function printForecastProfitBoard(&$object, &$listofreferent, $dates, $datee) {
        ...
        if($qualified && isset($margin)) {
            ...
            if (count($elementarray)>0 && is_array($elementarray))
            {
                ...
                if($key == 'propal' && ! empty($TLinkedOrder)) {
                    ...
                    print '<tr class="oddeven">';
                    // Module
                    print '<td class="left">'.$langs->trans('OrdersFormProposals').'</td>';
                    // Nb
                    print '<td class="right">'.$TLinkedOrder['nbOrder'].'</td>';
                    // Amount HT
                    print '<td class="right">';
                    if (! $qualifiedforfinalprofit) print '<span class="opacitymedium">'.$form->textwithpicto($langs->trans("NA"), $langs->trans("AmountOfInteventionNotIncludedByDefault")).'</span>';
    ...
    print '<tr class="liste_total">';
    print '<td class="right" colspan="2" >'.$langs->trans("ForecastProfit").'</td>';
    print '<td class="right" >'.price(price2num($balance_ht, 'MT')).'</td>';
    print '<td class="right" >'.price(price2num($balance_ttc, 'MT')).'</td>';
    print '</tr>';
}